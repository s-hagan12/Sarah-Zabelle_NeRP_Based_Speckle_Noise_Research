...
if __name__ == '__main__':
    ...
    with open(os.path.join(output_directory,'output.txt'), 'w') as f:
        ...
        if opts.pretrain:
            ...
            print('Load pretrain model: {}'.format(model_path), file=f)
        ...
        # Setup data loader
        print('Load image: {}'.format(config['img_path']), file=f)
        ...
        for it, (grid, image) in enumerate(data_loader):
            ...
            print(grid.shape, image.shape, file=f)

            # 2D parallel projection geometry
            thetas = torch.linspace(0, np.pi, config['num_proj']+1)[:-1].to(device)
            print(thetas, file=f)
            projs = ct_parallel_project_2d_batch(image, thetas)  # [bs, n, w, c]
            print(projs.shape, file=f)
            ...
            for iterations in range(max_iter):
                ...
                if (iterations + 1) % config['log_iter'] == 0:
                    ...
                    print("[Iteration: {}/{}] Train loss: {:.4g} | Train psnr: {:.4g}".format(iterations + 1, max_iter, train_loss, train_psnr), file=f)
                ...
                if iterations == 0 or (iterations + 1) % config['val_iter'] == 0:
                    ...
                    print("[Validation Iteration: {}/{}] Test loss: {:.4g} | Test psnr: {:.4g}".format(iterations + 1, max_iter, test_loss, test_psnr), file=f)
